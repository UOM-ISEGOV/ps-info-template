$("document").ready(function () {
  ...
  function showIneligible(reason) {
    hideFormBtns();
    $("#backButton").show(); // Επιτρέπουμε στον χρήστη να γυρίσει πίσω αν έκανε λάθος

    var errorHtml = `
      <div class="govgr-error-summary" role="alert" tabindex="-1">
        <h2 class="govgr-error-summary__title">Δεν είστε δικαιούχος</h2>
        <div class="govgr-error-summary__body">
          <p>${reason || "Δεν πληρούνται οι απαραίτητες προϋποθέσεις."}</p>
        </div>
      </div>
    `;
    $(".question-container").html(errorHtml);
  }
  ...
  function showEligible(outcomeType, reason) {
    hideFormBtns();
    $("#backButton").show(); // Επιτρέπουμε να πάει πίσω αν επιθυμεί

    var evidences = collectEvidences();

    var evidenceListHtml = "";
    if (evidences.length > 0) {
      evidenceListHtml = `
        <br />
        <h3 class="govgr-heading-m">Απαιτούμενα Δικαιολογητικά:</h3>
        <ul class="govgr-list govgr-list--bullet">
          ${evidences.map((e) => `<li>${e}</li>`).join("")}
        </ul>
      `;
    }
    ...
    var resultHtml = `
      <div id="resultWrapper">
        <div class="govgr-panel govgr-panel--confirmation">
          <h1 class="govgr-panel__title">Είστε δικαιούχος!</h1>
          <div class="govgr-panel__body">${reason}</div>
        </div>
        ${evidenceListHtml}
      </div>
    `;
